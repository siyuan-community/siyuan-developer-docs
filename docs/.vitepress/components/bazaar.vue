...
const getResources = async () => {
  return Promise.all([
    fetch(
      `${Constants.BazaarOSSServer}/bazaar@${bazaarHash.value}/stage/plugins.json`,
      { mode: "no-cors" }
    ).then((res) => res.json()),
    fetch(
      `${Constants.BazaarOSSServer}/bazaar@${bazaarHash.value}/stage/templates.json`,
      { mode: "no-cors" }
    ).then((res) => res.json()),
    fetch(
      `${Constants.BazaarOSSServer}/bazaar@${bazaarHash.value}/stage/themes.json`,
      { mode: "no-cors" }
    ).then((res) => res.json()),
    fetch(
      `${Constants.BazaarOSSServer}/bazaar@${bazaarHash.value}/stage/widgets.json`,
      { mode: "no-cors" }
    ).then((res) => res.json()),
  ]).then((arr) => {
...
const total = computed(() =>
  namedUserRepos.value.reduce((a, i) => (i.downloads || 0) + a, 0)
);
...
<template>
  <div class="devtool-plugin-tab" style="padding: 12px">
    ...
    <div class="center">
      ...
      <span style="margin: 6px 12px; width: 100%">
    ...
    <div>
      <div class="user-repo-container">
        <template v-for="p in namedUserRepos">
          <div class="user-repo fn__flex" v-if="p.package">
